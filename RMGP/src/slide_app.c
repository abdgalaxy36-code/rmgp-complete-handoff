...
static int slide_commit_virtual_base(uint64_t base, const char *source) {
  ...
  kaslr_base = base;
  kaslr_slide = base - KIMAGE_TEXT_BASE; ks_slide_delta = kaslr_slide;
  kaslr_done = 1;
...
#error Unsupported SLIDE_STACK_WRITER value
  ...
  /* a37x ground-truth probe: if the fops hijack landed, a fresh ashmem fd
     now has configfs_read_iter and pread returns >=0; a pristine fd has no
     read_iter at all and pread fails with EINVAL. */
  {
    int pfd = open(ashmem_path, O_RDWR | O_CLOEXEC);
    if (pfd < 0) {
      pr_warning("fops probe open errno=%d path=%s\n", errno, ashmem_path);
    } else {
      char pb[8];
      memset(pb, 0xAA, sizeof(pb));
      errno = 0;
      ssize_t pr = pread(pfd, pb, sizeof(pb), 0);
      int pe = errno;
      close(pfd);
      pr_info("fops probe pread=%zd errno=%d byte0=%02x verdict=%s\n",
              pr, pe, (unsigned char)pb[0],
              (pr >= 0) ? "HIJACK_LIVE" : "NO_HIJACK");
    }
  }
  atomic_store(&slide_route_done, 1);
...
static int slide_restore_physical_oracle(void) {
  ...
  return gate_restored && probe_restored;
    ...
    (defined(APP_FOPS_ORACLE_DIAG_ONLY) && APP_FOPS_ORACLE_DIAG_ONLY) || \
...
static int slide_commit_stext(uint64_t stext, const char *source) {
  ...
  kaslr_base = stext;
  kaslr_slide = slide; ks_slide_delta = kaslr_slide;
