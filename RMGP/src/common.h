...
#endif
/* a37x: /dev/ashmem<bootid> clone nodes (libcutils shim label) store the
   name RAW at area offset 0 -- no dev/ashmem/ prefix, verified by
   SET/GET_NAME round-trip on-device. Classic value was 11. */
...
void kernelsnitch_set_profile(struct kernelsnitch_shared_state *ks, size_t appended_futexes, size_t repeat_measurement, size_t average);
